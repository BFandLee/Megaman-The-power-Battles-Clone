...
NodeState BTAction_Gemini_Jump::Tick(Blackboard* bb)
{
    ...
    if (!_isJumpStarted)
    {
        ...
        bb->BossRigidBody->SetGrounded(false);
        bb->BossRigidBody
    ...
    {
        if (bb->BossAnimation->GetCurrentClipName() != L"Jump")
        {
            bb->BossAnimation->Play(L"Jump", true);
        }
    }
...
NodeState BTAction_Gemini_Idle::Tick(Blackboard* bb)
{
    ...
    if (bb->BossAnimation->GetCurrentClipName() != L"Idle")
    {
        bb->BossAnimation->Play(L"Idle", true);
    }
...
NodeState BTAction_Gemini_Move::Tick(Blackboard* bb)
{
    ...
    if (bb->BossAnimation->GetCurrentClipName() != L"Move")
    {
        bb->BossAnimation->Play(L"Move", true);
    }
