...
void AnimatorComponent::SyncFrom(const AnimatorComponent* other)
{
	if (other == nullptr)
		return;

	if (other->_currentClipName.empty())
		return;

	auto it = _clips.find(other->_currentClipName);
	if (it != _clips.end())
	{
		_currentClip = it->second;
		_currentClipName = other->_currentClipName;
		_currentFrame = other->_currentFrame;
		_accmulatedTime = other->_accmulatedTime;
		_bisFinished = other->_bisFinished;

		if (_currentClip != nullptr && !_currentClip->frames.empty() && _currentFrame >= (int32)_currentClip->frames.size())
		{
