...
public:
	...
	void SetCurrentFrame(int32 frame) { _currentFrame = frame; }

	float GetAccumulatedTime() const { return _accmulatedTime; }
	void SetAccumulatedTime(float time) { _accmulatedTime = time; }

	void SyncFrom(const AnimatorComponent* other);
};
